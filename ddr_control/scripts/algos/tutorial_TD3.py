...
class TD3:
    ...
    def save(self, path, AGENT):  # save trained weights
        ...
        tl.files.save_npz(self.q_net1.trainable_weights, extend_path('{}_model_q_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.q_net2.trainable_weights, extend_path('{}_model_q_net2.npz'.format(AGENT)))
        tl.files.save_npz(self.target_q_net1.trainable_weights, extend_path('{}_model_target_q_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.target_q_net2.trainable_weights, extend_path('{}_model_target_q_net2.npz'.format(AGENT)))
    ...
    def load(self, path, AGENT):  # load trained weights
        # path = os.path.join('model', '_'.join([ALG_NAME, AGENT]))
        extend_path = lambda s: os.path.join(path, s)
        tl.files.load_and_assign_npz(extend_path('{}_model_q_net1.npz'.format(AGENT)), self.q_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_q_net2.npz'.format(AGENT)), self.q_net2)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_q_net1.npz'.format(AGENT)), self.target_q_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_q_net2.npz'.format(AGENT)), self.target_q_net2)
