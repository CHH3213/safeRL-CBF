...
def constrains(State):
    ...
    ''' pursuer-evader'''
    sinp, cosp, sine, cose = tf.sin(th_p), tf.cos(th_p), tf.sin(th_e), tf.cos(th_e)
    ...
    # pv_K0 = 2.5
    def con_we1(we):
        return 2 * (Vp ** 2 + Ve ** 2 - 2 * Vp * Ve * tf.cos(th_p - th_e)) + 2 * wp * tf.einsum('i,i->', pv_Dp,
                                                                                                pv_Dot_vp) - \
               2 * we * tf.einsum('i,i->', pv_Dp, pv_Dot_ve) + \
               pv_K0 * (tf.einsum('i,i->', pv_Dp, pv_Dp) - r ** 2) + \
               2 * pv_K1 * (tf.einsum('i,i->', pv_Dp, pv_Dv))

    '''good - good'''
    sin_good, cos_good, sine, cose = tf.sin(th_good), tf.cos(th_good), tf.sin(th_e), tf.cos(th_e)
    ...
    # g_K0 = 2.5
    def con_we2(we):
        return 2 * (Vgood ** 2 + Ve ** 2 - 2 * Vgood * Ve * tf.cos(th_good - th_e)) + 2 * w_good * tf.einsum('i,i->',
                                                                                                             g_Dp,
                                                                                                             g_Dot_vp) - \
               2 * we * tf.einsum('i,i->', g_Dp, g_Dot_ve) + \
               g_K0 * (tf.einsum('i,i->', g_Dp, g_Dp) - r ** 2) + \
               2 * g_K1 * (tf.einsum('i,i->', g_Dp, g_Dv))
    ...
    # b_K0 = 2.5
    def con_we3(we):
        return 2 * Ve ** 2 + 2 * w_b * tf.einsum('i,i->', b_Dp, b_Dot_vp) - \
               2 * we * tf.einsum('i,i->', b_Dp, b_Dot_ve) + \
               b_K0 * (tf.einsum('i,i->', b_Dp, b_Dp) - r ** 2) + \
               2 * b_K1 * (tf.einsum('i,i->', b_Dp, b_Dv))
    ...
    def con_we4(we):
        return 2 * Ve ** 2 + 2 * w_w * tf.einsum('i,i->', Dp, Dot_vp) - \
               2 * we * tf.einsum('i,i->', Dp, Dot_ve) + \
               K0 * (tf.einsum('i,i->', Dp, Dp) - r_wall ** 2) + \
               2 * K1 * (tf.einsum('i,i->', Dp, Dv))
