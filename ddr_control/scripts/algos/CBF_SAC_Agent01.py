""" 
Soft Actor-Critic (SAC)-Control Barrier Function-qp layer
为智能体1的SAC
"""
...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done, safe):
        ...
        self.buffer[self.position] = (state, action, reward, next_state, done, safe)
        self.position = int((self.position + 1) % self.capacity)  # as a ring buffer

    def sample(self, BATCH_SIZE):
        batch = random.sample(self.buffer, BATCH_SIZE)
        state, action, reward, next_state, done, safe = map(np.stack, zip(*batch))  # stack for each element
        """ 
...
class PolicyNetwork(Model):
    ...
    def evaluate(self, actions, state_batch, epsilon=1e-6):
        ...
        action_batch = self.action_range * action_0
        ...
        action_0 = action_batch / self.action_range
    ...
    def get_action(self, actions, state, greedy=False):
        ...
        qp_state = np.concatenate((state[5:7] + state[0:2], [state[18]], state[0:3], actions[2],
                                   state[3:5], [state[17]], actions[1], state[13:15], state[15:17]))
        action = CBF(action.numpy()[0], qp_state)
        return action
...
class SAC:
    ...
    def __init__(
        ...
        self.soft_safe_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim)
        self.soft_safe_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim)
        self.target_soft_safe_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim)
        self.target_soft_safe_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim)
        ...
        self.soft_safe_net1.train()
        self.soft_safe_net2.train()
        self.target_soft_q_net1.eval()
        self.target_soft_q_net2.eval()
        self.target_soft_safe_net1.eval()
        self.target_soft_safe_net2.eval()
        ...
        self.target_soft_safe_net1 = self.target_ini(self.soft_safe_net1, self.target_soft_safe_net1)
        self.target_soft_safe_net1 = self.target_ini(self.soft_safe_net2, self.target_soft_safe_net2)

        self.soft_q_optimizer1 = tf.optimizers.Adam(SOFT_Q_LR)
        self.soft_q_optimizer2 = tf.optimizers.Adam(SOFT_Q_LR)
        self.soft_safe_optimizer1 = tf.optimizers.Adam(SOFT_Q_LR)
        self.soft_safe_optimizer2 = tf.optimizers.Adam(SOFT_Q_LR)
    ...
    def update(self, actions, batch_size, reward_scale=10., auto_entropy=True, target_entropy=-2,
               gamma=0.99,
               soft_tau=1e-2,
               safe_scale=10.):
        """ update all networks in SAC """
        state, action, reward, next_state, done, safe = self.replay_buffer.sample(batch_size)
        ...
        # Training Q Function        
        new_next_action, next_log_prob, _, _, _ = self.policy_net.evaluate(actions, next_state)
        # TODO：add qp layer,,next_log_prob如何处理
        # qp_state = np.concatenate((next_state[2][0:3], next_state[0][0:3], actions[2],
        #                            next_state[1][0:3], actions[1], next_state[0][0:2], next_state[0][0:2]))
        # new_next_action = CBF(new_next_action, qp_state)
        ...
        # TODO：add qp layer,,next_log_prob如何处理
        # qp_state = np.concatenate((state[2][0:3], state[0][0:3], actions[2],
        #                            state[1][0:3], actions[1], state[0][0:2], state[0][0:2]))
        # action = CBF(action, qp_state)
        q_input = tf.concat([state, action], 1)  # the dim 0 is number of samples

        # Training Safe Function
        target_safe_input = tf.concat([next_state, new_next_action], 1)
        target_safe_min = tf.minimum(
            self.target_soft_safe_net1(target_safe_input), self.target_soft_safe_net2(target_safe_input)
        ) - self.alpha * next_log_prob
        target_safe_value = safe + (1 - done) * gamma * target_safe_min  # if done==1, only reward
        safe_input = tf.concat([state, action], 1)  # input of q_net
        ...
        with tf.GradientTape() as safe1_tape:
            predicted_safe_value1 = self.soft_safe_net1(safe_input)
            safe_value_loss1 = tf.reduce_mean(tf.losses.mean_squared_error(predicted_safe_value1, target_safe_value))
        safe1_grad = safe1_tape.gradient(safe_value_loss1, self.soft_safe_net1.trainable_weights)
        self.soft_safe_optimizer1.apply_gradients(zip(safe1_grad, self.soft_safe_net1.trainable_weights))

        with tf.GradientTape() as safe2_tape:
            predicted_safe_value2 = self.soft_safe_net2(safe_input)
            safe_value_loss2 = tf.reduce_mean(tf.losses.mean_squared_error(predicted_safe_value2, target_safe_value))
        safe2_grad = safe2_tape.gradient(safe_value_loss2, self.soft_safe_net2.trainable_weights)
        self.soft_safe_optimizer2.apply_gradients(zip(safe2_grad, self.soft_safe_net2.trainable_weights))

        # Training Policy Function
        with tf.GradientTape() as p_tape:
            new_action, log_prob, z, mean, log_std = self.policy_net.evaluate(actions, state)
            # TODO:qp layer
            # qp_state = np.concatenate((state[2][0:3], state[0][0:3], actions[2],
            #                            state[1][0:3], actions[1], state[0][0:2], state[0][0:2]))
            # new_action = CBF(new_action, qp_state)

            new_q_input = tf.concat([state, new_action], 1)  # the dim 0 is number of samples
            new_safe_input = tf.concat([state, new_action], 1)
            """ implementation 1 """
            predicted_new_q_value = tf.minimum(self.soft_q_net1(new_q_input), self.soft_q_net2(new_q_input))
            predicted_new_safe_value = tf.minimum(self.soft_safe_net1(new_safe_input),
                                                  self.soft_safe_net2(new_safe_input))
            # """ implementation 2 """
            # predicted_new_q_value = self.soft_q_net1(new_q_input)
            # TODO:safe and critic weights problems
            # policy_loss = tf.reduce_mean(self.alpha * log_prob - predicted_new_q_value - predicted_new_safe_value)
        ...
        self.target_soft_safe_net1 = self.target_soft_update(self.soft_safe_net1, self.target_soft_safe_net1, soft_tau)
        self.target_soft_safe_net2 = self.target_soft_update(self.soft_safe_net2, self.target_soft_safe_net2, soft_tau)
    ...
    def save(self, path, AGENT):  # save trained weights
        ...
        tl.files.save_npz(self.soft_q_net1.trainable_weights, extend_path('{}_model_q_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.soft_q_net2.trainable_weights, extend_path('{}_model_q_net2.npz'.format(AGENT)))
        tl.files.save_npz(self.target_soft_q_net1.trainable_weights,
                          extend_path('{}_model_target_q_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.target_soft_q_net2.trainable_weights,
                          extend_path('{}_model_target_q_net2.npz'.format(AGENT)))
        tl.files.save_npz(self.soft_safe_net1.trainable_weights, extend_path('{}_model_safe_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.soft_safe_net2.trainable_weights, extend_path('{}_model_safe_net2.npz'.format(AGENT)))
        tl.files.save_npz(self.target_soft_safe_net1.trainable_weights,
                          extend_path('{}_model_target_safe_net1.npz'.format(AGENT)))
        tl.files.save_npz(self.target_soft_safe_net2.trainable_weights,
                          extend_path('{}_model_target_safe_net2.npz'.format(AGENT)))
        tl.files.save_npz(self.policy_net.trainable_weights, extend_path('{}_model_policy_net.npz'.format(AGENT)))
        np.save(extend_path('{}_log_alpha.npy'.format(AGENT)), self.log_alpha.numpy())  # save log_alpha variable

    def load(self, path, AGENT):  # load trained weights
        # path = os.path.join('model', '_'.join([ALG_NAME, ENV_ID]))
        extend_path = lambda s: os.path.join(path, s)
        tl.files.load_and_assign_npz(extend_path('{}_model_q_net1.npz'.format(AGENT)), self.soft_q_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_q_net2.npz'.format(AGENT)), self.soft_q_net2)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_q_net1.npz'.format(AGENT)), self.target_soft_q_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_q_net2.npz'.format(AGENT)), self.target_soft_q_net2)
        tl.files.load_and_assign_npz(extend_path('{}_model_safe_net1.npz'.format(AGENT)), self.soft_safe_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_safe_net2.npz'.format(AGENT)), self.soft_safe_net2)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_safe_net1.npz'.format(AGENT)),
                                     self.target_soft_safe_net1)
        tl.files.load_and_assign_npz(extend_path('{}_model_target_safe_net2.npz'.format(AGENT)),
                                     self.target_soft_safe_net2)
        tl.files.load_and_assign_npz(extend_path('{}_model_policy_net.npz'.format(AGENT)), self.policy_net)
        self.log_alpha.assign(np.load(extend_path('{}_log_alpha.npy'.format(AGENT))))  # load log_alpha variable
