...
import numpy as np
from CBF_SAC_Agent01 import SAC as SAC_Agent01
from CBF_SAC_Agent01 import ReplayBuffer as ReplayBuffer_Agent01
from CBF_SAC_Agent02 import SAC as SAC_Agent02
from CBF_SAC_Agent02 import ReplayBuffer as ReplayBuffer_Agent02
from CBF_SAC_Agent03 import SAC as SAC_Agent03
from CBF_SAC_Agent03 import ReplayBuffer as ReplayBuffer_Agent03
import argparse
import os
...
def main():
    ...
    parser.add_argument('--saveData_dir', default="./save/sac_add_obs_{}/data".format(
        time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())),
                        help="directory to store all experiment data")
    parser.add_argument('--testData_dir', default="./save/sac_add_obs_2021-11-30-18:26:30/data",
                        help="directory to store all experiment data")
    parser.add_argument('--saveModel_dir', default='./save/sac_add_obs_{}/models'.format(
        time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())),
                        help="where to store/load network weights")
    parser.add_argument('--load_dir', default='./save/sac_add_obs_2021-11-30-18:26:30/models',
                        help="where to load network weights")

    parser.add_argument('--checkpoint_frequency', default=5, type=int,
    ...
    agent_1 = SAC_Agent01(s_dim, a_dim, a_bound, HIDDEN_DIM, replay_Buffer[0],
                          args.learning_rate,
                          args.learning_rate)
    agent_2 = SAC_Agent02(s_dim, a_dim, a_bound, HIDDEN_DIM, replay_Buffer[1],
                          args.learning_rate,
                          args.learning_rate)
    agent_3 = SAC_Agent03(s_dim, a_dim, a_bound, HIDDEN_DIM, replay_Buffer[2],
                          args.learning_rate,
                          args.learning_rate)
    ...
    t_state = time.time()
    reward_txt_name = '/home/YHS/ros_ws/src/IEEE_TAI_DDR/ddr_control/scripts/algos/save_txt/sac_qp_reward_st_{}.txt'.format(
        time.strftime(
            "%Y-%m-%d-%H:%M:%S", time.localtime()))
    reward_txt_name_ddr0 = '/home/YHS/ros_ws/src/IEEE_TAI_DDR/ddr_control/scripts/algos/save_txt/sac_qp_reward_st_ddr0_{}.txt'.format(
        time.strftime(
            "%Y-%m-%d-%H:%M:%S", time.localtime()))
    reward_txt_name_ddr1 = '/home/YHS/ros_ws/src/IEEE_TAI_DDR/ddr_control/scripts/algos/save_txt/sac_qp_reward_st_ddr1_{}.txt'.format(
        time.strftime(
            "%Y-%m-%d-%H:%M:%S", time.localtime()))
    count_txt_name = '/home/YHS/ros_ws/src/IEEE_TAI_DDR/ddr_control/scripts/algos/save_txt/sac_qp_count_st_{}.txt'.format(
        time.strftime(
            "%Y-%m-%d-%H:%M:%S", time.localtime()))
    if not args.testing:
        print('train!!!!!!!!!!!!!!!!')
        for episode in range(args.episodes):
            ...
            for t in range(max_steps):
                ...
                # print(state)
                for i, agent in enumerate(agents):
                    action = agent.policy_net.get_action(actions, state[i], greedy=False)
            ...
            # if episode % 200 == 0:
            #     model_dir = './save/sac_safe_temp_{}/models'.format(time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime(time.time())))
        ...
        for episode in range(50):
            ...
            for step in range(400):
                for i, agent in enumerate(agents):
                    ...
                    action = agent.policy_net.get_action(actions, state[i], greedy=True)
                    actions[i] = action
                bad_index = 2
            ...
            if not os.path.exists(model_dir):
                os.makedirs(model_dir)
            sio.savemat(model_dir + '/sac_cbf_test_{}_{}.mat'.format(time_start, time.strftime("%d-%H:%M:%S",
                                                                                               time.localtime())),
        ...
        sio.savemat(args.testData_dir + '/sac_nocbf_nod_test_{}.mat'.format(time.strftime("%d-%H:%M:%S",
                                                                                          time.localtime())),
                    {'episode_state': state_each_episode, 'count_wall': count_wall, 'count_agent': count_agent})
