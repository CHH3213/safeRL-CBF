...
class RCBF_SAC(object):
    ...
    def select_action(self, state,other_state, dynamics_model, evaluate=False, warmup=False):
        ...
        safe_action = self.get_safe_action(state, other_state, action, dynamics_model)
    ...
    def update_parameters(self, memory, batch_size, updates, dynamics_model, memory_model=None, real_ratio=None):
        ...
        batch_size : int
        updates : int
        dynamics_model : 主要利用里面的obs与state之间的转换
        ...
        if memory_model and real_ratio:
            state_batch, other_state_batch,action_batch, reward_batch, next_state_batch, mask_batch, t_batch, next_t_batch = memory.sample(batch_size=int(real_ratio*batch_size))
            state_batch_m, other_state_batch_m, action_batch_m, reward_batch_m, next_state_batch_m, mask_batch_m, t_batch_m, next_t_batch_m = memory_model.sample(
                batch_size=int((1-real_ratio) * batch_size))
            state_batch = np.vstack((state_batch, state_batch_m))
            other_state_batch_m = np.vstack((other_state_batch_m, other_state_batch_m))
            ...
            state_batch, other_state_batch,action_batch, reward_batch, next_state_batch, mask_batch, t_batch, next_t_batch = memory.sample(batch_size=batch_size)
        ...
        with torch.no_grad():
            ...
            if self.diff_qp:  # Compute next safe actions using Differentiable CBF-QP
                next_state_action = self.get_safe_action(next_state_batch,other_state_batch, next_state_action, dynamics_model)
        ...
        if self.diff_qp:  # Compute safe action using Differentiable CBF-QP
            pi = self.get_safe_action(state_batch,other_state_batch, pi, dynamics_model)
    ...
    def get_safe_action(self, obs_batch, other_state_batch, action_batch, dynamics_model):
