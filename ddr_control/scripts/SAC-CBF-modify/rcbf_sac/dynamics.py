...
class DynamicsModel:
    ...
    def append_transition(self, state_batch, u_batch, next_state_batch, t_batch=None):
        ...
        disturbance_batch = (next_state_batch - state_batch - self.env.dt * (
                    self.get_f(state_batch, t_batch) + (self.get_g(state_batch, t_batch) @ u_batch).squeeze(
                -1))) / self.env.dt
        ...
        for i in range(state_batch.shape[0]):
            ...
            self.disturbance_history['disturbance'][self.history_counter % self.max_history_count] = disturbance_batch[
                i]
    ...
    def load_disturbance_models(self, output):
        ...
        try:
            ...
            for i in range(self.n_s):
                self.disturb_estimators.append(
                    GPyDisturbanceEstimator(self.train_x, self.train_y[:, i], MAX_STD[self.env.dynamics_mode][i],
                                            device=self.device))
