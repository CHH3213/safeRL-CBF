...
from utils import to_tensor, prRed
...
class CBFQPLayer:
    ...
    def get_cbf_qp_constraints(self, state_batch, action_batch, mean_pred_batch, sigma_pred_batch):
        ...
        if self.env.dynamics_mode == 'Unicycle':
            ...
            G = torch.zeros((batch_size, num_constraints, n_u + 1)).to(
            ...
            h[:, :num_cbfs] = gamma_b * (hs ** 3) + (
                        torch.bmm(dhdps, f_ps + mu_ps) - torch.bmm(torch.abs(dhdps), sigma_ps) + torch.bmm(
                    torch.bmm(dhdps, g_ps), action_batch)).squeeze(-1)
            ...
            # Inequality constraints (G[u, eps] <= h)
            h[:, 0] = Lffh13 - LfDfh13 + (gamma_b + gamma_b) * h13_dot + gamma_b * gamma_b * h13 + torch.bmm(Lgfh13,
                                                                                                             action_batch).squeeze()
            h[:, 1] = Lffh15 - LfDfh15 + (gamma_b + gamma_b) * h15_dot + gamma_b * gamma_b * h15 + torch.bmm(Lgfh15,
                                                                                                             action_batch).squeeze()
