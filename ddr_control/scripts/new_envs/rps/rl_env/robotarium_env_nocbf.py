...
class RobotariumEnv(core.Env):
    def __init__(self, args=None):
        ...
        # obs 包括：最近障碍物的x,y（1个障碍物，共2维），目标点x,y,最近agent的x,y
        # 障碍物位置
        locations = 1
        self.hazards_locations = np.array([ [1.5, 0., 0.], [0.0, 1.5, 0.],[locations, locations, 0.]])
        
        # self.hazards_locations = np.array([[-1.5, 0., 0.], [1.5, 0., 0.], [0.0, 1.5, 0.], [0.0, -1.5, 0.],[1.0,1.0,0],
        ...
        self.goal_train_random = np.array([[0, 0], [2, 2]]) #原训练
    ...
    def _reward_done(self, index):
        ...
        # reward += 0.00125 * dist_list[ind]  # 050801
        # reward += 0.0012 * dist_list[ind]  # 2
        # reward += 0.03 * dist_list[index]  # 1
        now_dist = dist_list[ind]
        if self.last_distSqr < now_dist:
            reward += 0.22 
        else:
            reward -= 0.22 
        self.last_distSqr = copy.deepcopy(dist_list[ind])
        ...
        # # reward += 0.002 * dist_list[ind]  # 050701
        # reward += 0.005 * dist_list[ind]  # 050702
        # reward += 0.0005 * dist_list[ind]  # 050801
        # reward += 0.0012 * dist_list[ind]  # 2
        # reward += 0.03 * dist_list[index]  # 1
        ...
        for idx in range(np.size(other_agent_s, 0)):
            ...
            if distSqr < self.hazards_radius ** 2:
                ...
                # if self.last_distSqr_2 < distSqr:
                #     reward += 0.215  
                # else:
                #      reward-=0.215 #4
                reward += 0  # 无动态障碍设置
                # reward += 0.001 * distSqr  
        self.last_distSqr_2 = copy.deepcopy(distSqr)
        ...
        if self.args.mode != "train":
            ...
            if self.goal_met(index):
                ...
                # reward -= 0.05 * dist_goal
                # reward += 10 * (self.last_goal_dist - dist_goal)
                if self.last_goal_dist - dist_goal>0:
                    reward +=0.2
                else:
                    reward -=0.2

        self.last_goal_dist = copy.deepcopy(dist_goal)
    ...
    def reset(self, is_show_figure=False):
        ...
        if self.args.mode == "train":
            ...
            a = np.array([[10 * np.random.rand() - 5, 10. *
                         np.random.rand() - 5, 6.28 * np.random.rand() - 3.14]]).T
            self.initial_conditions = a
            for _ in range(1, self.agent_number):
                self.initial_conditions = np.concatenate((self.initial_conditions, np.array(
                    [[10 * np.random.rand() - 5, 10. *
                      np.random.rand() - 5, 6.28 * np.random.rand() - 3.14]]).T), axis=1)
        else:
            # a = np.array([[0., 0., 0]]).T
            # self.initial_conditions = a
            # for i in range(1, self.agent_number):
        ...
        self.last_distSqr = 10000
        self.last_distSqr_2 = 10000
    ...
    def get_obs(self, index):
        ...
        # temp_obs = np.concatenate((temp_obs, [np.exp(-goal_dist)])) # 添加一维,变7维
        ...
        # # Normalize
        # rel_o_s = other_agent_s[idx, 0:2] - self_state[:2]
        # # R = np.array([[np.cos(self_state[2]), -np.sin(self_state[2])],
        # #              [np.sin(self_state[2]), np.cos(self_state[2])]])
        # # rel_o_s = np.matmul(rel_o_s, R)
        ...
        # temp_obs = np.concatenate((temp_obs, self.hazards_locations[ind,:2] - self_state[:2]))
