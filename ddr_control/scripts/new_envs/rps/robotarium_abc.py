            ...
            for i in range(number_of_robots):
                if i == 0:
                    p = patches.RegularPolygon(self.poses[:2, i], 4, math.sqrt(2) * self.robot_radius,
                                               self.poses[2, i] + math.pi / 4, facecolor='#FFD700', edgecolor='k')
                if i >= 1:
                    p = patches.RegularPolygon(self.poses[:2, i], 4, math.sqrt(2) * self.robot_radius,
                                               self.poses[2, i] + math.pi / 4, facecolor='#0028ff', edgecolor='k')

                rled = patches.Circle(self.poses[:2, i] + 0.75 * self.robot_radius * np.array(
                ...
                lled = patches.Circle(self.poses[:2, i] + 0.75 * self.robot_radius * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i])) + \
                    0.015 * np.array(
                        (-np.sin(self.poses[2, i] + math.pi / 2), np.cos(self.poses[2, i] + math.pi / 2)))), \
                                      self.robot_radius / 5, fill=False)
                rw = patches.Circle(self.poses[:2, i] + self.robot_radius * np.array(
                    (np.cos(self.poses[2, i] + math.pi / 2), np.sin(self.poses[2, i] + math.pi / 2))) + \
                                    0.04 * np.array(
                    (-np.sin(self.poses[2, i] + math.pi / 2), np.cos(self.poses[2, i] + math.pi / 2))), \
                                    0.02, facecolor='k')
                lw = patches.Circle(self.poses[:2, i] + self.robot_radius * np.array(
                    (np.cos(self.poses[2, i] - math.pi / 2), np.sin(self.poses[2, i] - math.pi / 2))) + \
                ...
                # chh add 2022-03-16
                # txt = [self.poses[0, i], self.poses[1, i], i]
                # self.txts.append(txt)
                # axes_txt = self.axes.text(*txt)
                # axes_txt.set_position((self.poses[0, i], self.poses[1, i]))
            # Draw area
