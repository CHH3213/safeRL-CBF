...
class TAI_Env:
    def __init__(self):
        ...
        self.target = np.array([[0, 0], [3, 3], [3, 3]])
        # TODO：根据算法调整
    ...
    def step(self, action_n):
        ...
        for i in range(self.agent_number):
            ...
            self.target[i] = np.array([0, 0]) if self.is_origin[i] else np.array([3, 3])
    ...
    def _get_obs(self):
        ...
        for i, position in enumerate(positions_all):
            ...
            obs_one_agent = np.concatenate((position, [attitudes_all[i]], delta_pos, delta_dist, theta_array, np.array(self.target[i])-np.array(position)))
    ...
    def _compute_reward(self, agent_index, target):
        ...
        reward, done = 0, False
        if dist_agent2target < 0.6 and self.is_origin[agent_index]:
            reward += 300
            if agent_index == 0 or agent_index == 1:
                self.is_origin[agent_index] = False
                self.count_reach += 1
            # done = True
        elif dist_agent2target < 0.6 and not self.is_origin[agent_index]:
            reward += 100
