...
"""
==========IEEE_TAI_DDR===============
@File: test_env:测试走完整个来回
@Time: 2021/11/13 上午11:37
@Author: chh3213
...
class TAI_Env:
    def __init__(self):
        ...
        self.obs_dim = 17  # 每个agent的observation的维度
        ...
        for i in range(self.agent_number):
            ...
            self.observation_space.append(spaces.Box(low=np.array([-5, -5, -np.pi, -5, -5, -5, -5, -10, -10, -np.pi, -np.pi, -10, -10, -10, -10, -10, -10]),
                                                     high=np.array([5, 5, np.pi, 5, 5,  5, 5, 10, 10, np.pi,  np.pi, 10, 10, 10, 10, 10, 10]), dtype=np.float32))
        # robot properties
    ...
    def reset(self):
        ...
        self.count_reach = 0
        self.is_origin = [False, False, False]
        self.target = np.array([[2, 2], [2, 2], [2, 2]])
        self.gazebo_reset.resetWorld()
        self.gazebo_reset.reset_agent_state_test(ddr_list=self.ddr_list)
    ...
    def step(self, action_n):
        ...
        for i in range(self.agent_number):
            ...
            self.target[i] = np.array(
                [0, 0]) if self.is_origin[i] else np.array([2, 2])
        safe_n.append(0)
    ...
    def _get_obs(self):
        ...
        for i, ddr_name in enumerate(self.ddr_name):
            [model_position, model_attitude, _, _,
                _] = self.gazebo_reset.get_state(ddr_name)
        ...
        for i, position in enumerate(positions_all):
            ...
            obs_one_agent = np.concatenate((position, [attitudes_all[i]], delta_pos, delta_dist, theta_array, np.array(self.target[i])-np.array(position), barrier_pos, wall_pos))
    ...
    def _take_action(self, actions):
        ...
        state1 = np.concatenate((self.obs[2][0:3], self.obs[0][0:3], actions[2], self.obs[1][0:3], actions[1], barrier_pos0[0:2],wall_pos0[0:2]))
        state2 = np.concatenate((self.obs[2][0:3], self.obs[1][0:3], actions[2], self.obs[0][0:3], actions[0], barrier_pos1[0:2],wall_pos1[0:2]))
        state3 = np.concatenate((np.array([10,10,10]), self.obs[2][0:3], [0], np.array([10,10,10]), [0], barrier_pos1[0:2],wall_pos1[0:2]))
        # print(state2)
        ...
        for i, action in enumerate(actions):
            ...
            if i == 2:
                vel.linear.x = self.v_linear*0.9
        ...
        for _ in range(1):
            ...
            self.rate.sleep()        # self.gazebo_reset.pausePhysics()
        # rospy.logdebug("\nEnd Taking Action\n")
    ...
    def _compute_reward(self, agent_index, target):
        ...
        reward, done = 0, False
        if dist_agent2target < 0.3 and self.is_origin[agent_index]:
            reward += 300
            if agent_index == 0 or agent_index == 1:
                self.is_origin[agent_index] = False
                self.count_reach += 1
                self.done_reach = True

            # done = True
        elif dist_agent2target < 0.3 and not self.is_origin[agent_index]:
            reward += 100
        ...
        for d in self.obs_dict[agent_index]['delta_dist']:
            if d < 0.25:
                reward -= 1000
                self.is_origin[agent_index] = False
                # self.gazebo_reset.reset_agent_state(
                #     [self.ddr_list[agent_index]])
                if agent_index == 2:
    ...
    def _safe_constrains(self, good_obs, bad_obs, good_action, bad_action):
        ...
        return 2 * (Vp ** 2 + Ve ** 2 - 2 * Vp * Ve * np.cos(th_p - th_e)) + \
    ...
    def _compute_safe(self, good_obs, bad_obs, good_action, bad_action):
        ...
        """
        h = self._safe_constrains(
            good_obs, bad_obs, good_action, bad_action)  # CBF
