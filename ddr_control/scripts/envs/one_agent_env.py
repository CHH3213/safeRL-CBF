...
class TAI_Env:
    ...
    def step(self, action_n):
        ...
        reward, done = self._compute_reward(0, self.target)
    ...
    def _get_obs(self):
        ...
        for i, position in enumerate(positions_all):
            ...
            obs_one_agent_dict = {'position': position, 'euler': [attitudes_all[i]], 'target': self.target}
            obs_one_agent = np.concatenate((position, [attitudes_all[i]], np.array(
                self.target) - np.array(position), barrier_pos, wall_pos))
